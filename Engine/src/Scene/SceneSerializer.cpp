...
namespace Engine
{
    ...
    namespace
    {
        // "Ajouter ou récupérer" : ApplyJsonToEntity doit pouvoir s'appliquer à une
        // entité qui a déjà le component, là où AddComponent refuse ce cas.
        template <typename T>
        T &Ensure(Entity entity)
        {
            return entity.HasComponent<T>() ? entity.GetComponent<T>() : entity.AddComponent<T>();
        }
    }

    // Sérialisation d'une entité seule, extraite de la boucle de Serialize() : l'éditeur
    // en a besoin entité par entité, c'est sur cette forme qu'une instance se compare à
    // sa scène source, propriété par propriété.
    json SceneSerializer::EntityToJson(Entity entity)
    {
        json entityJson;
    ...
    // Applique un JSON d'entité à une entité existante. Idempotent : un component déjà
    // présent est mis à jour au lieu d'être réajouté, ce qui permet de s'en servir aussi
    // bien au chargement que pour révoquer une surcharge d'instance.
    void SceneSerializer::ApplyJsonToEntity(const json &entityJson, Entity entity)
    {
        ...
        {
            auto &sc = Ensure<SpriteRendererComponent>(entity);
        ...
        {
            auto &rb = Ensure<RigidBodyComponent>(entity);
        ...
        {
            auto &bc = Ensure<BoxColliderComponent>(entity);
        ...
        {
            auto &cc = Ensure<CircleColliderComponent>(entity);
        ...
        {
            auto &ac = Ensure<AudioComponent>(entity);
        ...
        {
            auto &si = Ensure<SceneInstanceComponent>(entity);
        ...
        {
            auto &cc = Ensure<CameraComponent>(entity);
    ...
    json SceneSerializer::ToJson() const
    {
        ...
        for (auto entityHandle : view)
        {
            ...
            json entityJson = EntityToJson(entity);
    ...
    bool SceneSerializer::FromJson(const json &root)
    {
        ...
        for (auto &entityJson : root["Entities"])
        {
            ...
            ApplyJsonToEntity(entityJson, entity);
        }
